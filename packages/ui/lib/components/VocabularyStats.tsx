...
export const VocabularyStats = ({ items }: VocabularyStatsProps) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
      ...
      <div className={cn('rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800')}>
        ...
        <p className={cn('text-2xl font-bold text-gray-900 dark:text-gray-100')}>{total}</p>
      </div>
      ...
      <div className={cn('rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800')}>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-sm">
            <span className={cn('flex items-center gap-2 text-gray-700 dark:text-gray-300')}>
              <span className="block h-2 w-2 rounded-full bg-green-500"></span>
              Mastered
            </span>
            <span className={cn('font-medium text-gray-900 dark:text-gray-100')}>{byLevel.mastered}</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className={cn('flex items-center gap-2 text-gray-700 dark:text-gray-300')}>
            ...
            <span className={cn('font-medium text-gray-900 dark:text-gray-100')}>{byLevel.learning}</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className={cn('flex items-center gap-2 text-gray-700 dark:text-gray-300')}>
            ...
            <span className={cn('font-medium text-gray-900 dark:text-gray-100')}>{byLevel.struggling}</span>
          </div>
        </div>
      </div>
          ...
          {languages.slice(0, 3).map(([lang, count]) => {
            ...
            return (
              <div key={lang} className="flex items-center justify-between text-sm">
                <span className={cn('font-medium text-gray-700 dark:text-gray-300')}>{lang}</span>
