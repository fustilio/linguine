...
export const TextEvaluator = ({ isLight, onAnalyze, result, isAnalyzing = false }: TextEvaluatorProps) => {
  ...
  const getColorForLevel = (level: number): string => {
    if (level === 0) return 'bg-red-500 text-red-200';
    if (level <= 2) return 'bg-yellow-500 text-yellow-200';
    if (level <= 3) return 'bg-blue-500 text-blue-200';
  ...
  return (
    <div
          ...
          <select
            ...
            className={cn(
              ...
              isLight
                ...
                : 'border-gray-600 bg-gray-900 text-gray-100 focus:ring-blue-500',
            )}>
            {LANGUAGES.map(lang => (
          ...
          <textarea
            ...
            className={cn(
              ...
              isLight
                ...
                : 'border-gray-600 bg-gray-900 text-gray-100 placeholder-gray-400 focus:ring-blue-500',
            )}
          />
        </div>
      ...
      {result && (
        <div className={cn('space-y-3 rounded-lg border p-4', isLight ? 'border-gray-200' : 'border-gray-600')}>
          <div>
            <h4 className={cn('mb-2 text-sm font-semibold', isLight ? 'text-gray-900' : 'text-gray-100')}>Results</h4>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className={isLight ? 'text-gray-600' : 'text-gray-400'}>Total words:</span>
                <span className={cn('ml-2 font-medium', isLight ? 'text-gray-900' : 'text-gray-100')}>
                  {result.totalWords}
                </span>
              </div>
              <div>
                <span className={isLight ? 'text-gray-600' : 'text-gray-400'}>Known:</span>
                <span className={cn('ml-2 font-medium', isLight ? 'text-gray-900' : 'text-gray-100')}>
                  {result.knownWords} ({result.knownPercentage.toFixed(1)}%)
                </span>
              </div>
              <div>
                <span className={isLight ? 'text-gray-600' : 'text-gray-400'}>Struggling:</span>
                <span className={cn('ml-2 font-medium', isLight ? 'text-gray-900' : 'text-gray-100')}>
                  {result.strugglingWords}
                </span>
              </div>
              <div>
                <span className={isLight ? 'text-gray-600' : 'text-gray-400'}>Mastered:</span>
                <span className={cn('ml-2 font-medium', isLight ? 'text-gray-900' : 'text-gray-100')}>
          ...
          <div className={cn('rounded border p-3', isLight ? 'border-gray-200' : 'border-gray-600')}>
            <p className={cn('mb-2 text-xs font-medium', isLight ? 'text-gray-700' : 'text-gray-300')}>
              Word breakdown:
            </p>
